...
function animate1(element,target){//传入两个参数，一个是移动的对象，一个是目标距离
    clearInterval(element.setId);//每次进入清理计时器
    element.setId=setInterval(function(){
        var current=element.offsetLeft;//当前位置
        var step=(target-current)/10;//每次移动的步数
        step=step>0?Math.ceil(step):Math.floor(step);
        current+=step;
        if(Math.abs(target-current)<Math.abs(step)){
            clearInterval(element.setId);//停止移动，清理计时器
